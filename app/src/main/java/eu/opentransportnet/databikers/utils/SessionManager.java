...
import eu.opentransportnet.databikers.activities.LoginActivity;
...
public class SessionManager {
    ...
    public static final String KEY_BIKE_TYPE = "bike_type";
    ...
    public boolean createLoginSession(User user) {
        ...
        mEditor.putInt(KEY_BIKE_TYPE, user.getBikeType());
    ...
    public boolean saveBikeType(int bikeType) {
        mEditor.putInt(KEY_BIKE_TYPE, bikeType);
        return mEditor.commit();
    }
    ...
    public User getUser() {
        ...
        user.setBikeType(mPref.getInt(KEY_BIKE_TYPE, -1));
    ...
    public void logoutUser() {
        File tackJsonDir = new File(mContext.getFilesDir() + "/" + Const.STORAGE_PATH_INFO);
        File trackCsvDir = new File(mContext.getFilesDir() + "/" + Const.STORAGE_PATH_TRACK);
        File reportDir = new File(mContext.getFilesDir() + "/" + Const.STORAGE_PATH_REPORT);
        File[] routesJson = tackJsonDir.listFiles();
        File[] routesCsv = trackCsvDir.listFiles();
        File[] reports = reportDir.listFiles();

        if (routesJson.length > 0 || routesCsv.length > 0 || reports.length > 0) {
            AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
            builder.setTitle(mContext.getString(eu.opentransportnet.databikers.R.string.delete_local_files))
                    .setMessage(mContext.getString(eu.opentransportnet.databikers.R.string.delete_local_files_message))
                    .setPositiveButton(mContext.getString(eu.opentransportnet.databikers.R.string.yes),
                    ...
                    .setNegativeButton(mContext.getString(eu.opentransportnet.databikers.R.string.no),
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int id) {
                                }
                            });
